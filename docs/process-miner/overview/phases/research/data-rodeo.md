...
In this phase, the main aim is to use process mining tools available to create accurate [process indicators](../../glossary#process-indicator-pi) that provide adequate ways to measure and understand the processes and their changes. 

The objectives of this session are to **define** and **analyze** the process, **validate** the values obtained and its quality (applying corrections if necessary), and **train** health professionals in getting the understanding of using the application and analyze the data by themselves; preparing the [logs](../../glossary#log); and applying process mining [discovery](../../glossary#discovery), [conformance](../../glossary#conformance) and [enhancement](../../glossary#enhancement) techniques to achieve the best process indicators for the problem at hand.
...
**(7)**	Once the IPI is defined, it is distributed between health professionals as the PMApp dashboard ([PMApp](../../glossary#interactive-process-mining-application-pmapp) plus a [runner](../../glossary#runner)).
...
**(1) Factories.** The process miner selects the data sources using [factories](../../../ipmt/user-guide/experiment-designer/factories/csv) to extract the data and to create the process mining [log](../../glossary#log) by collecting the available [events](../../glossary#event). Some of these factories require the creation of an ingestor, which is implemented in the [Ingestor Editor](../../../ipmt/user-guide/experiment-designer/ingestor-editor).
...
**(3) Log processors.** Once the log is prepared, it is run a [process discovery](../../glossary#discovery) algorithm to create the **graphical model** from the log. 
...
The data rodeo process is implemented in the [Experiment Designer](../../../ipmt/user-guide/experiment-designer/intro).
...
The ingestion process is implemented in the [Ingestion Editor](../../../ipmt/user-guide/experiment-designer/ingestor-editor) accessible from the [Experiment Designer](../../../ipmt/user-guide/experiment-designer/intro).